...
describe "syslog packet parser" do
  ...
  it "parse some valid packets" do
    p = SyslogProtocol.parse("<34>2016-06-21T16:02:26.871 mymachine su: 'su root' failed for lonvick on /dev/pts/8")
    ...
    p.time.should.equal Time.parse("2016-06-21T16:02:26.871")

    p = SyslogProtocol.parse("<13>2016-06-21T16:02:26.871 10.0.0.99 test: Use the BFG!")
  ...
  it "PRI with preceding 0's shall be considered invalid" do
    p = SyslogProtocol.parse("<045>2016-06-21T16:02:26.871 space_station my PRI is not valid")
    p.facility.should.equal 1
    p.severity.should.equal 5
    p.pri.should.equal 13
    p.hostname.should.equal 'unknown'
    p.content.should.equal "<045>2016-06-21T16:02:26.871 space_station my PRI is not valid"
  end

  it "allow the user to pass an origin to be used as the hostname if packet is invalid" do
    p = SyslogProtocol.parse("<045>2016-06-21T16:02:26.871 space_station my PRI is not valid", '127.0.0.1')
    p.facility.should.equal 1
    p.severity.should.equal 5
    p.pri.should.equal 13
